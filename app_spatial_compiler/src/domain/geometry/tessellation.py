...
def _find_maximal_cut(
    nodes: Sequence[SpatialNode], axis: str
) -> tuple[int | None, float, list[SpatialNode]]:
...
def get_spatial_blocks(
    nodes: Sequence[SpatialNode], min_dx: float = -1.0, min_dy: float = -1.0
    ...
    if y_ratio >= x_ratio:
        return get_spatial_blocks(
            y_sorted[:y_idx], min_dx, min_dy
        ...
        return get_spatial_blocks(left_nodes, min_dx, min_dy) + get_spatial_blocks(
            right_nodes, min_dx, min_dy
        )
...
def detect_graphical_voids(
    nodes: Sequence[SpatialNode], page_bounds: tuple[float, float, float, float]
    ...
    for i in range(len(sorted_y) - 1):
        ...
        if gap > 100.0:
            voids.append(
                (page_bounds[0], sorted_y[i].y1, page_bounds[2], sorted_y[i + 1].y0)
            )
    return voids
