...
from app_spatial_compiler.src.domain.geometry.tessellation import (
    detect_graphical_voids,
    get_spatial_blocks,
)
from app_spatial_compiler.src.application.use_cases.compile_document import (
    CompileDocumentUseCase,
)
from app_spatial_compiler.src.infrastructure.adapters.vision_encoder import (
    VisionEncoderAdapter,
)
from app_spatial_compiler.src.infrastructure.adapters.pdf_extractor import (
    PDFExtractorAdapter,
)
...
class CompositeSpatialCompiler:
    ...
    def compile_graph(self, nodes: Sequence[SpatialNode]) -> MarkdownAST:
        if not nodes:
            return MarkdownAST(content="", metadata={})
        page_median_h = statistics.median(
...
def compile(
    ...
    pdf: Annotated[
        Optional[str], typer.Option("--pdf", help="Path to PDF file")
    ] = None,
) -> None:
