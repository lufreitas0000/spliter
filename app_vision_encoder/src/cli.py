...
app = typer.Typer(
    help="Vision Encoder: Map continuous physical image tensors to discrete semantic AST nodes."
)
console = Console()
...
def encode(
    ...
    use_fake: bool = typer.Option(
        False,
        "--use-fake",
        help="Bypass VLM inference, use deterministic Fake adapter.",
    ),
    use_api: bool = typer.Option(
        False, "--use-api", help="Delegate inference to external HTTP API."
    ),
) -> None:
    """Executes the semantic encoding pipeline on a target physical image tensor."""
    console.print(
        Panel(
            f"Target Tensor: [cyan]{image_path}[/cyan]", title="Vision Encoder Engine"
        )
    )

    if not image_path.exists():
        console.print(
            f"[bold red]Fatal Error:[/bold red] Tensor artifact not found at {image_path}"
        )
    ...
    if use_fake:
        console.print(
            "[yellow]Notice: Injecting deterministic FakeVisionEncoderAdapter.[/yellow]"
        )
        ...
        console.print(
            "[yellow]Notice: Injecting ExternalAPIAdapter (HTTP Socket Delegation).[/yellow]"
        )
        api_key = os.environ.get("OPENAI_API_KEY")
        if not api_key:
            console.print(
                "[bold red]Fatal Error:[/bold red] OPENAI_API_KEY environment variable undefined."
            )
            raise typer.Exit(code=1)
        ...
        console.print(
            "[green]Notice: Injecting LocalQuantizedAdapter (4-bit VRAM Allocation).[/green]"
        )
        console.print("[dim]Lazy-loading PyTorch/Transformers context...[/dim]")
        from app_vision_encoder.src.adapters.local_quantized import (
