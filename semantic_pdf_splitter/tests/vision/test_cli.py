...
from semantic_pdf_splitter.vision.cli import app
...
def test_cli_encode_with_fake_adapter(synthetic_image_tensor: Path) -> None:
    ...
    assert "Fake deterministic extraction for" in result.stdout
    assert "FakeVisionEncoderAdapter" in result.stdout
